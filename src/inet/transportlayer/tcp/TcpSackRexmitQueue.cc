...
//        }
        ...
        if (fromSeqNum != toSeqNum) { //rarely called? this is called if the block does not exist AND fromSeqNum is not the end of the current queue
            ...
            if(region.sacked){
                m_sackedOut += region.endSeqNum - region.beginSeqNum;
            }
...
void TcpSackRexmitQueue::skbSent(uint32_t seqNum, simtime_t m_firstSentTime, simtime_t m_lastSentTime, simtime_t m_deliveredTime, bool m_isAppLimited, uint32_t m_delivered)
{
    ...
    region.m_isAppLimited = m_isAppLimited;
